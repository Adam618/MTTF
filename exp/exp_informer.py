...
class Exp_Informer(Exp_Basic):
    ...
    def test(self, setting, args):
        ...
        mae,mse,rmse,res,r_2 = metric(preds, trues)
        print('mae:{}, mse:{}, rmse:{}, res:{}, r_2:{}'.format(mae,mse,rmse,res,r_2))

        np.save(folder_path+'metrics.npy', np.array([mae,mse,rmse,res,r_2]))
        np.save(folder_path+'pred.npy', preds)
        np.save(folder_path+'true.npy', trues)

        folder_path1 = './results/'
        result = pd.DataFrame([[datetime.datetime.now(),  args.img_path,args.is_img_embed,args.d_model,args.learning_rate, args.batch_size,args.seq_len, args.label_len, args.pred_len,args.img_pred_len, args.lag_step,args.attn,args.distil,mse, mae, rmse,res,r_2]])
        result.to_csv('./results/tem_result.csv',mode='a', header=False)
        return
    ...
    def predict(self, setting, args, load=True):
        ...
        result = 't{}_{}_dt{}_ie{}_dm_{}_lr{}_bs{}_sl{}_ll{}_pl{}_ipl{}_lg{}_at{}_dt{}_mse:{}, mae:{}'.format(
            datetime.datetime.now(), 'pred', args.img_path, args.is_img_embed, args.d_model, args.learning_rate,
            args.batch_size, args.seq_len, args.label_len, args.pred_len, args.img_pred_len, args.lag_step,args.attn,args.distil,mse, mae)
        print(result)
        f.write(result)
        f.write('\n')
        f.close()
...
#                 img_data = torch.cat([img_data, img_one], dim=1).float().to(self.device)
        ...
        dec_inp = batch_y[:,:self.args.label_len,:]
